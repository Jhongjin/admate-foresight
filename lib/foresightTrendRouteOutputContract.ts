...
export interface TrendRouteIndustry {
  industry: string;
  trends: TrendRoutePoint[];
}
...
function normalizeTrendPoint(value: unknown): TrendRoutePoint | null {
...
export function normalizeTrendsRouteOutput(value: unknown): TrendRouteIndustry[] {
  if (!Array.isArray(value)) return [];

  return value.flatMap((item): TrendRouteIndustry[] => {
    if (!isRecord(item)) return [];

    const industry = readSafeIndustryLabel(item.industry);
    const trends = Array.isArray(item.trends)
      ? item.trends
          .map(normalizeTrendPoint)
          .filter((point): point is TrendRoutePoint => point !== null)
      : [];

    if (!industry || trends.length === 0) return [];
