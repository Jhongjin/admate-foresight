            ...
            <div className="relative" ref={siteRef}>
              ...
              {siteOpen ? (
                <div
                  ...
                  className="absolute right-0 mt-2 w-[400px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-[10px] border border-[#D7DCE3] bg-white p-2 text-[#172033] shadow-[0_20px_56px_rgba(16,24,32,0.16)] max-sm:fixed max-sm:left-4 max-sm:right-4 max-sm:top-14 max-sm:mt-0 max-sm:w-auto max-sm:max-w-none"
                >
                  <p className="px-3 py-2 text-[11px] font-bold uppercase tracking-[0.18em] text-[#68707C]">ADMATE SUITE</p>
                  ...
                  {siteItems.map((site) => (
                    <Link
                      ...
                      href={site.href}
                      role="menuitem"
                      className="grid min-h-[66px] grid-cols-[48px_minmax(0,1fr)] items-center gap-3 rounded-[8px] px-3 py-2.5 transition-colors hover:bg-[#F4F7FB] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#172033]"
                    >
                      <span className="grid h-12 w-12 shrink-0 place-items-center rounded-[8px] border border-[#D7DCE3] bg-[#F8F6F1]">
                        <SiteIcon name={site.icon} />
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2 text-[15px] font-bold leading-tight text-[#172033]">
                          {site.label}
                          {site.active ? <span className="rounded-[6px] bg-[#FFF3D8] px-1.5 py-0.5 text-[11px] font-bold text-[#7A5518]">현재</span> : null}
                        </span>
                        <span className="mt-0.5 block text-[13px] font-medium leading-[18px] text-[#68707C]">{site.description}</span>
                      </span>
                    </Link>
                  ))}
                </div>
